...
const where = (query) => {
	...
	const queryFilter = JSON.parse(query);
...
const order = (data, orderBy, type) => {

	if(orderBy == undefined || type == undefined) {
		return data;
	};

	if((availableOrderFields.indexOf(orderBy.toLowerCase()) < 0) || (availableOrderTypes.indexOf(type.toLowerCase())) < 0) {
		return false;
	};
	...
	switch(orderBy){
		...
		case 'is_on_trip':
		case 'distance':
			result = orderByNonLocation(data, orderBy, type)
			break;
